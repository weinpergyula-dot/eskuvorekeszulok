      ...
      <div className={cn("flex items-center gap-3 bg-[#FCFCFC] rounded-xl border shadow-sm hover:shadow-md transition-all px-4 py-3", tc.borderBase, tc.borderHover)}>
        ...
        >
          {provider.avatar_url ? (
            ...
            <img src={storageImageUrl(provider.avatar_url, 128)} alt={provider.full_name} className="w-full h-full object-cover" />
          ) : (
      ...
      {inCarousel && (
        <div className="relative flex flex-col items-center pt-8 px-7 pb-6" style={{ backgroundColor: headerBg }}>
          ...
          >
            {provider.avatar_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={storageImageUrl(provider.avatar_url, 256)} alt={provider.full_name} className="w-full h-full object-cover" />
            ) : (
        ...
        <div className="relative flex flex-col sm:flex-row" style={{ backgroundColor: headerBg }}>
          ...
          <div className="flex flex-col items-center justify-center pt-11 pb-3 sm:pt-12 sm:pb-5 sm:pl-10 sm:pr-5 sm:border-r sm:border-white/40">
            ...
            >
              {provider.avatar_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={storageImageUrl(provider.avatar_url, 192)} alt={provider.full_name} className="w-full h-full object-cover" />
              ) : (
      ...
      {hasGallery && !inCarousel && (
        <div className="overflow-hidden border-t border-b border-gray-200" style={{ height: "84px", backgroundColor: "white" }}>
          {galleryUrls.length === 1 ? (
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={storageImageUrl(galleryUrls[0], 224)} alt="Galéria" className="w-full h-full object-cover" draggable={false} />
            </button>
          ) : (
            <div
              className="flex"
            ...
            >
              {[...galleryUrls, ...galleryUrls].map((url, i) => (
                ...
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={storageImageUrl(url, 224)} alt="" className="w-full h-full object-cover" draggable={false} />
                </button>
              ))}
            </div>
          )}
