...
export async function GET() {
  ...
  const enriched = (messages ?? []).map((m) => {
    ...
    return {
      ...
      sender_avatar_url: isOwn ? null : (providerMap[m.sender_id]?.avatar_url ?? null),
      ...
      recipient_avatar_url: isOwn ? (recipientProviderMap[m.recipient_id]?.avatar_url ?? null) : null,
    };
  });
